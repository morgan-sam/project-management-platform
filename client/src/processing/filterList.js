export const filterList = (filterOptions, taskList) => {
    ...
    for (let i = 0; i < filterFunctions.length; i++)
        filteredList = filterFunctions[i](filterOptions, filteredList);
    return filteredList;
};

export const filterListDate = (filterOptions, list) =>
    list.filter((el) => filterOptions.date <= el.date);
...
export const filterListTeams = (filterOptions, list) => {
    ...
    return list.filter((el) => {
        if (teams.includes('all')) return true;
            ...
            teamMatch === 'AND' &&
            el.teams.every((team) => teams.includes(team))
        )
            return true;
        else if (
            teamMatch === 'OR' &&
            el.teams.some((team) => teams.includes(team))
        )
            return true;
        else return false;
    });
...
export const filterListCompletion = (filterOptions, list) => {
    return list.filter((el) => {
        ...
        else if (filterOptions.completion === 'complete' && el.completed)
            return true;
        else if (filterOptions.completion === 'incomplete' && !el.completed)
            return true;
        else return false;
    });
};
