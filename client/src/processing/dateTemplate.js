...
const convertTemplateToInstructions = (template) => {
  ...
  while (template.length) {
    ...
    for (let i = 0; i < regexList.length; i++) {
      let { value, newTemplate } = retrieveInstructionFromTemplate(
        template,
        regexList[i].regex
      );
      if (newTemplate !== null) {
  ...
  if (instructions[0].type !== "date")
    return "ERROR: TEMPLATE MUST START WITH DATE";
...
const regexList = [
  ...
  },
  {
    type: "date",
    regex: new RegExp("^\\$\\{t\\}"),
    default: parseISOToDateObj(getDayFromTodayAsISO()),
  },
...
const retrieveInstructionFromTemplate = (template, regex) => {
  ...
  if (!matches) return { value: null, newTemplate: null };
    ...
    return {
      value: regex.exec(template).groups
        ? regex.exec(template).groups
        : regex.exec(template)[0],
...
const convertTsFromTemplate = (instructions) =>
  instructions.map((el) =>
    el.type === "date"
      ? { type: "date", value: getDateWithExactValues(el.value) }
      : el
  );
...
const interpretInstructions = (instructions, taskCount) => {
  ...
  for (let task = 0; task < taskCount; task++) {
    ...
    for (let i = 0; i < instructions.length; i++) {
      ...
      if ((type === "date" || type === "algebra") && previous === null)
        previous = instructions[i];
      else if (type === "operator")
        operator = interpretOperatorInstructions(operator, value);
...
const sumInstructions = (current, previous, operator, task) => {
  ...
  if (type === "algebra")
    return {
      value: calculateDateWithAlgebra(previous.value, operator, value, task),
      type: "date",
    };
  else if (type === "date") {
    if (previous.type === "date")
      return {
        value: addSubtractDates(previous.value, value, operator),
        type: "date",
      };
    else if (previous.type === "algebra")
      return {
        value: calculateDateWithAlgebra(value, operator, previous.value, task),
        type: "date",
      };
  }
};
...
const addSubtractDates = (dateOne, dateTwo, operator) => {
  ...
  date = new Date(
    date.setFullYear(date.getFullYear() + parseInt(dateTwo.year) * sign)
  );
...
const getDateFromAlgebra = (date, algebra, product) => {
  ...
  else if (algebra.match(/w/g))
    date = date.setDate(date.getDate() + product * 7);
  else if (algebra.match(/m/g)) date = addMonths(date, product);
  else if (algebra.match(/y/g))
    date = date.setFullYear(date.getFullYear() + product);
  return date;
};
