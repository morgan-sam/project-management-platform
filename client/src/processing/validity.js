const charAtStartOrEnd = (string, char) =>
  string.match(new RegExp(`^[^${char}].*[^${char}]$`, "g")) === null;
const stringConsecutiveDots = (string) =>
  string.match(new RegExp(/\.\./g)) !== null;
...
export const checkIfEmailValid = (email) => {
  ...
  if (charAtStartOrEnd(local, ".") || stringConsecutiveDots(local))
    return false;
