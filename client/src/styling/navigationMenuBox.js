...
const getExactRemPosition = (menuPos) => {
  const topPositionRem =
    (menuPos.slice(1).reduce((a, b) => a + b, 0) + 1) * BOX_HEIGHT_REM;
  const leftPositionRem = (menuPos.length + menuPos[0] - 2) * BOX_WIDTH_REM;
  if (menuPos.length === 1)
    return { left: `${menuPos[0] * BOX_WIDTH_REM}rem`, top: "0" };
...
const adjustForBorderPixels = (menuPos, position) => {
  ...
  if (menuPos.length === 1) {
    ...
    left = `${
      leftPx + (2 - menuPos.length - menuPos[0]) * BOX_BORDER_WIDTH_PX
    }px`;
    top = `${
      topPx +
      (-menuPos.slice(1).reduce((a, b) => a + b, 0) - 1) * BOX_BORDER_WIDTH_PX
    }px`;
  }
  return { top, left };
};
