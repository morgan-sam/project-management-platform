...
const Task = (props) => {
  ...
  const getDataCellText = (type) => {
    ...
    else if (type === "date" || type === "deadline")
      return parseISOToLittleEndian(item[type]);
  ...
  const getDataCell = (type, i) => {
    ...
    const cellStyle = {
      ...cellStyles[type],
      ...(selected ? getHighlightCellStyle(themeColor) : null),
    };
    const dragFns =
      type === "task" || type === "selected" ? dragSelectionFunctions : null;
    return (
      <DataCell
        key={i}
  ...
  const getAllDataCells = () =>
    getTrueObjVals(visibleColumns).map((type, i) => getDataCell(type, i));
