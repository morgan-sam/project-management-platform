...
const TableEntries = (props) => {
    ...
    const memoizedSetEntryCompletion = useCallback(
        (item, completed) => setEntryCompletion(item, completed),
        []
    );
    ...
    useEffect(() => {
        const combinedArrs = combineRemoveBothDuplicates(
            drag.previous,
            drag.current
        );
    ...
    useEffect(() => {
        if (drag.held && !pressedKeys.includes('Control')) {
            const newCurrent = drag.previous.includes(drag.start)
                ? []
                : [drag.start];
            setDrag({ ...drag, previous: [], current: newCurrent });
        } else if (drag.current.length > 0) {
            const nextPrev = combineRemoveBothDuplicates(
                drag.previous,
                drag.current
            );
