...
import {
    ...
    rightSideArrowStyle,
    checkboxStyle
} from 'styling/navigationMenuBox';
...
const NavigationMenu = (props) => {
    ...
    const singleMenuBox = (el, menuPos) => {
        const { name, action, enabled, checkbox, boxColor } = el;
        const hovered = menuPos.toString() === hover.toString();
        const buttonState = { hovered, enabled, boxColor };
        const boxStyle = {
        ...
        const boxClick = () => {
            ...
            else if (enabled !== false && checkbox === undefined)
                setMenusOpen([]);
        };
        return (
            <div
            ...
            >
                {name}
                {menuPos.length > 1 && el.sub && (
                    <div style={rightSideArrowStyle}>▶</div>
                )}
                {checkbox !== undefined && (
                    <Checkbox
                        className="navMenu"
    ...
    const multipleBoxes = (el, menuPos) =>
        el.sub
            ? el.sub.map((el, i) => menuDropdownContainer(el, [...menuPos, i]))
            : null;
    ...
    const whileDropdownOpenClick = (e) =>
        e.target.className === 'navMenu' ? null : setMenusOpen([]);
        ...
        if (sub)
            return (
                ...
                >
                    ...
                    <div>
                        {menusOpen[menuPos.length - 1] ===
                            menuPos[menuPos.length - 1] && sub
    ...
    const mainRowOfBoxes = () =>
        props.menus.map((el, i) => menuDropdownContainer(el, [i]));

    return (
        <div
            style={{
                ...getParentContainer(props.menus.length),
                ...props.style
            }}
        >
            {mainRowOfBoxes()}
